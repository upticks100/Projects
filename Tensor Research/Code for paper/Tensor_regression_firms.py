...
GRID_SEARCH = {
    ...
    # Model Parameters
    "RANK_REGRESS": [15, 18], 
    "REG_W": [0.8, 1.0]    # Strong regularization
}
...
if __name__ == "__main__":
    ...
    for (L, r_imp) in data_configs:
        ...
        for params in matching_combos:
            ...
            # Validation Split (Last 30% of Training)
            val_idx = int(len(X_tr) * 0.7)
            
            try:
                # FIX #3: Ensure random state is passed clearly
                model = CPRegressor(weight_rank=r_reg, reg_W=reg_w, n_iter_max=300, 
                                    tol=1e-4, verbose=0, random_state=SEED)
                model.fit(X_tr[:val_idx], Y_tr[:val_idx])
                
                preds = model.predict(X_tr[val_idx:])
                
                # FIX #1: Robust Flattening for Scoring
                mask_val_flat = Mask_tr[val_idx:].reshape(-1)
                y_true_flat   = Y_tr[val_idx:].reshape(-1)
                ...
                r2 = 1.0 - np.sum((y_true_valid - y_pred_valid)**2) / sst
                
                log(f"  Model: Rank={r_reg}, Reg={reg_w} -> Val R2: {r2:.4f}")
