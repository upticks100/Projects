"""
compare_pure_cp_flat_vs_ridge_L2_L4.py

Pure CP (Flat) vs Ridge baseline, for BOTH lookback periods L=2 and L=4
(and for both MODE=LEVELS/SURPRISE), using the SAME data prep + CV + pooled
mask-aware R² used in Pure_CP_Flat.py.

No command-line args. Just run:
  python compare_pure_cp_flat_vs_ridge_L2_L4.py
"""
...
def run_one_combo(study: optuna.Study, cache_path: Path, mode: str, L: int) -> Tuple[float, float, float]:
    ...
    print(f"\n=== PURE CP FLAT vs RIDGE | MODE={mode} L={L} ===")
    print(
    ...
    for fold_idx, (tr_idx, val_idx) in enumerate(tscv.split(X_cv)):
        ...
        # -------------------------
        # Ridge baseline (flat, per feature, RidgeCV)
        # trained on SAME filtered rows
        # -------------------------
        ...
        r2_ridge = evaluate_model(Y_val_final, y_ridge_final, M_val_final)
        ...
        cp_model.fit(X_tr_tens_c, Y_target_scaled)

        preds_scaled = cp_model.predict(X_val_tens_c)
        y_cp_final = (preds_scaled * y_rms) + y_mean_tr
        r2_cp = evaluate_model(Y_val_final, y_cp_final, M_val_final)
...
def main():
    ...
    db_path = Path("/student/mcnama53/research_pure_flat.db")
    study_name = "research_pure_flat_v1"
    ...
    print("\n===== FINAL SUMMARY (Pure CP Flat vs Ridge) =====")
