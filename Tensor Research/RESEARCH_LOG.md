...
Nothing in main.tex touched — new file `Paper_Draft/main_v2.tex`.

### What was done
- Pre-prediction (Masoud's) text preserved verbatim EXCEPT exhibit updates to
  the clean MFI v2 rebuild (audit Finding 1): tensor 499x40x140, density 74.0%,
  Tucker [67,40,20] observed error 5.45% (was 22.2% on polluted v1), rho
  MFI↔FCIX 0.318 (was 0.33), independence table L_n=0.546/I_n=0.237 with v2
  permutation criticals. Two paragraphs describing the MFI figure's episodes
  updated to match the v2 series shape (2006-09 peak cluster, 2020Q1 spike,
  2024 climb). Added the missing FCIX source citation (Ataei et al. 2021,
  Physica A) — the old draft never cited it.
- New figures (nothing overwritten): `Figures/Fig_QMFI_v2.pdf`,
  `Figures/Fig_Cross_Corr_Quarters_v2.pdf`, generated by new script
  `Code for paper/regen_prepred_figures_v2.py` from pre_prediction_cache/mfi_v2/.
  Relative_Error figure switched to its .pdf twin (tectonic can't do EPS).
- Rewrite zone (prediction section onward), all numbers wired from artifact
  files (cited in tex comments):
  * Sec 4 Predictive Modeling: honest architecture (baseline+gamma*CP booster,
    FE and ridge-OOF cells, Optuna space rank 1-25 / REG_W 1e-2..1e5 /
    gamma 0-2, calendar-fixed 2021Q1 holdout). Results table = rank-1 trials
    from v3_holdout_ext aggregate_summary (FE +.048/+.043, ridge +.019/+.011,
    20/20 trials positive). Old fabricated-looking persistence table REPLACED
    by the per-feature stationarity result (vr_stat corr -0.45..-0.48).
  * Sec 5 Transfer: 499-firm locked-hyperparam table (delta +4/4, residual
    edge ~40% of mega-cap size) + Gram-identity low-mem CP fitter paragraph.
  * Sec 6 Economic content: event-study battery (return null 4/4, vol signal
    ridge-cells, IV subsumption, straddle=VRP); veer framework + H1 table
    (slope +0.020 4/4, PD translation 29-56% relative); IG CDS null +
    DD-intact composition check; HY universe + H-HY table (confirmed 4/4,
    ~-6.5bp/1sd); subordinate paragraph: HY straddle partial (L4-only,
    secondary 0/4), equity-return null, error-clustering null. Efficiency-
    gradient framing throughout.
  * Conclusion + abstract rewritten around the full arc; keywords updated.
- references.bib: appended 9 entries (ataei2021theory, zhou2013tensor,
  hoff2015multilinear, lock2018tensor, fama1973risk, bharath2008forecasting,
  benjamini2001control, akiba2019optuna, kossaifi2019tensorly — the last was
  cited in the OLD draft but missing from the bib all along).
- Compile: tectonic 0.15.0 installed to ~/bin (user approved). main_v2.tex
  compiles clean: 24 pages, zero undefined citations. Note: class needs the
  [pdflatex] documentclass option under tectonic/xetex.

### Files
  Paper_Draft/main_v2.tex (new), Paper_Draft/main_v2.pdf,
  Paper_Draft/Figures/{Fig_QMFI_v2.pdf, Fig_Cross_Corr_Quarters_v2.pdf},
  Code for paper/regen_prepred_figures_v2.py, references.bib (append-only).
