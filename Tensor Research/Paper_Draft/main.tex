...
We begin by computing the sample cross-correlation between the quarterly time series $\{\mathrm{MFI}_t\}$ and $\{\mathrm{FCIX}_t\}$ over a range of lags. Figure~\ref{Fig:Cross_Corr_Quarterly} illustrates the resulting correlation coefficients from $-10$ to $+10$ quarters. The strongest association is contemporaneous, with a peak correlation of approximately $0.33$ at lag zero, indicating a moderate near-term alignment between price-based volatility and variation in accounting fundamentals.
...
\begin{table}[ht]
	...
	\begin{tabular}{lcc}
		...
		\midrule
		Test statistic & 0.544 & 0.242 \\
		\addlinespace
		\textbf{Critical thresholds} & & \\
		\quad 1\% level   & 0.459 & 0.175 \\
		\quad 5\% level   & 0.420 & 0.147 \\
		\quad 10\% level  & 0.403 & 0.134 \\
		\bottomrule
	\end{tabular}
\end{table}
