...
Table~\ref{Independence_Test} reports the results of both tests applied to $\{\mathrm{FCIX}_t\}_{t=1}^{140}$ and $\{\mathrm{MFI}_t\}_{t=1}^{140}$, using $m_n = m_n' = 6$ equal-frequency discretization bins per dimension. Critical thresholds are obtained from 10,000 random permutations of the paired quarterly series.
...
\end{scope}
\end{tikzpicture}
\caption{Mask-aware Tucker imputation for sparse fundamentals. To resolve missing entries without discarding sparse features, the incomplete input tensor $\mathbfcal{X}_t \in \mathbb{R}^{N \times F \times L}$ is approximated by a dense core tensor $\mathbfcal{H} \in \mathbb{R}^{R_1 \times R_2 \times L}$ contracted with orthogonal factor matrices $\mathbf{A}$, $\mathbf{B}$, and $\mathbf{C}$ along each respective mode. The validated ranks are $(R_1,R_2,L)=(2,2,2)$ for the two-quarter lookback and $(4,4,4)$ for the four-quarter lookback.}
...
A predictive gain established on fifty mega-caps could in principle be an artifact of that particularly well-measured corner of the market. We therefore refit all four locked cells---hyperparameters frozen, no re-tuning of any kind---on the full cross-section of the top 499 firms by 2024Q4 market value, using fundamentals through 2026Q2 and the same calendar-fixed split. The declared gate for this exercise was directional: if the CP increment collapsed to zero or below in all four cells, that collapse would itself be the finding.

% Numbers: prediction_new/results/v3_holdout_499_20260706/transfer_check_499.csv
\begin{table}[!htbp]
\centering
\caption{Universe transfer at locked hyperparameters: pooled mask-aware test $R^2$ at 498 firms (22 test windows), against the 50-firm development deltas.}
...
On the 499-firm universe (roughly 6{,}900 scored events per cell over 18 test quarters), we pre-registered a single headline hypothesis: \emph{persistent cash-flow over-performance against the tensor forecast predicts improving credit quality}, measured as the change in a naive Merton distance-to-default \citep{bharath2008forecasting} from two trading days before the announcement to 63 trading days after, controlling for the pre-event level and pre-trend. The test is a Fama--MacBeth regression \citep{fama1973risk} across announcement quarters, with a partial rank information coefficient as a secondary check, and a declared success criterion of a positive slope at one-sided $p<0.05$ in at least two of four cells.

% Numbers: veer_report_*_499.txt; PD translation: h1_pd_translation_499.csv
The hypothesis was confirmed; Table~\ref{Tab:H1_DD} reports the four cells. The Fama--MacBeth slope is $+0.020$ in \emph{all four} (a striking stability), with one-sided $p = 0.033$ and $0.040$ in the two fixed-effects cells, $p = 0.060$ and $0.071$ in the ridge cells, and a positive partial rank IC ($t \approx +2.1$ to $+2.6$) everywhere.

\begin{table}[!htbp]
\centering
\caption{Pre-registered credit test at 499 firms: Fama--MacBeth slope of the post-announcement change in distance-to-default on the cash-flow drift signal, with pre-event level and pre-trend controls ($\approx$6{,}900 events per cell; declared sign positive, one-sided $p$; success bar: $p<0.05$ in $\geq 2$ of 4 cells).}
\label{Tab:H1_DD}
\begin{tabular}{llcccc}
\toprule
...
\bottomrule
\end{tabular}
\end{table} In economic units, a one-standard-deviation cash-flow drift maps to a distance-to-default improvement of $+0.05$ to $+0.07$; evaluated on the panel's distance-to-default distribution, this is a $29\%$ to $56\%$ \emph{relative} reduction in the model-implied physical default probability, growing with the firm's initial credit quality. We emphasize the honest caveat immediately: for this investment-grade mega/large-cap universe the absolute default probabilities are astronomically small, so a large relative change need not be worth anything in traded prices---the question the next subsection was designed to answer. The pre-registered secondary hypothesis (that the joint set of veer signals adds out-of-sample explanatory power for post-announcement changes in implied volatility) passed formally in three of four cells but with increments of at most $+0.003$ in $R^2$; we treat it as economically negligible, consistent with the option-subsumption result above.
...
To test whether the credit information is priced, we extended the confirmed hypothesis to market credit-default-swap spreads: five-year single-name CDS (senior unsecured, XR/XR14 documentation) from Markit, matched to 169 of the 499 firms with a name-validated link. The declared target was the change in log spread over the same event window, with the same controls; expected sign negative (over-performance tightens spreads). The result is a \emph{clean null in all four cells}: slopes are indistinguishable from zero ($|t| < 0.15$), amounting to $\pm 0.03$ basis points per one-standard-deviation signal at the sample's 56 basis-point median spread. Crucially, a composition check re-ran the confirmed distance-to-default regression on exactly the CDS-covered subsample: the slope is intact and slightly larger ($+0.024$ to $+0.026$, $t > 2$). The same firm-events whose model-implied credit quality improves show no movement in their traded spread. For investment-grade mega-caps, every traded price we test---equity around announcements, options on volatility, and now CDS on credit---already embeds what the tensor forecast errors know; only model-implied quantities respond. An exploratory cut foreshadowed the resolution: within the investment-grade panel, the distance-to-default effect concentrates in the highest spread tercile in all four cells, suggesting the channel prices where default risk is material.

% Universe: build_hy_universe.py; numbers: v3_holdout_hy_20260707/cds_h1_translation_hy.csv
We therefore constructed a dedicated high-yield/crossover universe and declared the hypothesis once more before touching the model. Starting from all 970 U.S. five-year senior CDS reference entities in Markit, we retained the 298 with median spreads of at least 150 basis points and linked them to CRSP/Compustat with normalized-name matching plus a hand-audited whitelist and blacklist (mapping financing subsidiaries to listed parents and discarding private or defunct entities), yielding 128 linked issuers, of which 113 pass the pipeline's universe gate. The accepted set has a median five-year spread of 289 basis points against 56 in the investment-grade sample---genuine crossover/high-yield credit. One design limitation is inherited and disclosed: the universe conditions on survival to 2024Q4, so the very blow-ups where the signal might be strongest are excluded, biasing against finding an effect. The full pipeline was re-run at locked hyperparameters; the transfer gate passed in three of four cells (the CP increment turns negative in the fixed-effects $L=4$ cell---the first such collapse anywhere, consistent with CP structure weakening as fundamentals get noisier down the quality spectrum).

\begin{table}[!htbp]
\centering
\caption{Pre-registered high-yield CDS test: Fama--MacBeth slope of the post-announcement change in log five-year CDS spread on the cash-flow drift signal, with pre-event spread level and pre-trend controls (900 events, 90 CDS-covered firms, 13 quarters per cell; declared sign negative, one-sided $p$). Basis-point impact evaluated at the 282 bp sample median spread.}
\label{Tab:HY_CDS}
\begin{tabular}{llcccc}
\toprule
...
Table~\ref{Tab:HY_CDS} reports the pre-registered outcome: the hypothesis is confirmed in \emph{all four cells} against a declared bar of two. In crossover and high-yield names, a one-standard-deviation persistent cash-flow over-performance against the tensor forecast predicts approximately $2.3\%$ of spread tightening---about 6.5 basis points at the median spread---over the following quarter, controlling for the spread level and its pre-trend, with negative partial rank ICs in all four cells. The same signal that moved only a model-implied credit metric in the investment-grade universe moves the traded market price of credit here. Together, the two CDS experiments trace an efficiency gradient: mega-cap investment-grade prices are fully efficient with respect to this information set, while high-yield credit incorporates it with a measurable lag. The sample is small (90 firms, 13 quarters) and the four cells are correlated, so we present this as one robust pre-registered finding rather than four.

Finally, three subordinate results complete the honest accounting. First, an options-tradeability battery was pre-registered on the high-yield universe (the same straddle-proxy long--short construction that failed at mega-caps): the primary criterion---a positive and bootstrap-significant long--short straddle mean in at least two of four cells---was met exactly at the declared bar, but only in the two $L=4$ cells, and the secondary criterion (per-pair significance incremental to implied volatility) failed in all four; we read this as a lumpy, architecture-dependent sort-level edge, clearly weaker than the CDS result, and do not claim a second tradeable leg. Second, the drift signal does \emph{not} predict forward equity returns at 499 firms (Fama--MacBeth $t < 0.8$ everywhere; tercile long--short spreads positive but insignificant), closing the equity route in this universe for both the magnitude and error-direction signals. Third, forecast errors are essentially idiosyncratic: clustering firms by error co-movement recovers no sectoral structure (adjusted Rand index versus GICS below $0.04$ in every cell) and the first principal component explains under $4\%$ of error variance---the veer signals are firm-specific news, not a missed common factor.
...
Second, we showed the same tensor structure is predictive. A low-rank CP tensor regression, deployed as a booster over per-feature ridge and fixed-effects baselines with strict out-of-fold discipline, improves out-of-time forecasts of next-quarter fundamentals in every locked configuration, on both a 50-firm development universe and the full 499-firm cross-section at frozen hyperparameters. The gain concentrates precisely on non-stationary, trending line items and on periods of cross-sectional co-movement---the regimes where per-feature linear models are mis-specified and shared low-rank structure exists to be exploited.

Third, we asked whether markets price what the tensor knows, under a pre-registration discipline that commits to hypotheses, criteria, and interpretations before each test. The answer is a coherent efficiency gradient. In mega-cap investment-grade names, every traded price we examined---announcement returns, option-implied volatility, and CDS spreads---already embeds the information in the tensor's forecasts and forecast errors; only model-implied quantities (a Merton distance-to-default) respond, and a large relative improvement in an infinitesimal default probability is worth nothing at the spread. In crossover and high-yield credit, where default risk is material, the pre-registered test confirmed in all four model configurations that persistent cash-flow over-performance against the tensor forecast predicts economically meaningful spread tightening---roughly 6.5 basis points per standard deviation per quarter at the median spread---controlling for spread level and momentum. The framework's marginal economic value thus lives exactly where theory says information should be slowest to price: lower down the credit spectrum, in the market's least-arbitraged corner.
