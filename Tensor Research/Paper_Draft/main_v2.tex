...
\abstract{
We introduce the Market Fundamentals Index (MFI), a novel metric constructed from a tensor representation of firm-level accounting data and smoothed via Tucker decomposition. The MFI captures time-varying dispersion in market fundamentals, and we demonstrate its strong statistical dependence on a price-based financial chaos index (FCIX). Building on this tensor framework, we develop a predictive pipeline that imputes missing fundamentals through mask-aware Tucker decomposition and forecasts the full next-quarter panel via low-rank CANDECOMP/PARAFAC regression deployed as a booster over fixed-effects and per-feature ridge baselines. On a calendar-fixed out-of-time holdout the tensor improves on a fully tuned ridge benchmark, its gains concentrating on non-stationary line items, and the improvement survives a locked-hyperparameter transfer to the full 499-firm selection universe. A sequence of hypotheses specified ex ante then traces where markets price this information: in mega-cap investment-grade names, announcement returns, option-implied volatility, and CDS spreads all subsume the tensor's signals, while in a dedicated high-yield/crossover universe, persistent cash-flow over-performance against the tensor forecast predicts economically meaningful CDS spread tightening (approximately 6.5 basis points per standard deviation per quarter) in all four model configurations---evidence consistent with slower incorporation of this information where default risk is material.
}


\keywords{multilinear tensor regression, Tucker decomposition, CANDECOMP/PARAFAC, market fundamentals, volatility index, credit default swaps, event study, market efficiency}
...
In total, over 600 distinct quarterly and annual features were considered. Through extensive search and evaluation, a 40-feature subset $\mathit{F}^\star$ was identified, as detailed in Table~\ref{Fundamentals}. The affinity propagation algorithm applied to $\mathbf{D}$ constructed from $\mathit{F}^\star$ yielded 156 clusters, just two fewer than the GICS-defined sub-industry count. This near match supports the claim that $\mathit{F}^\star$ provides a structurally faithful representation of economic heterogeneity across the market. As a supplementary check, Table~\ref{Tab:Affinity_GICS_Metrics} reports direct partition-similarity metrics against GICS levels on the canonical affinity-propagation design matrix; the agreement is positive but imperfect, so we interpret the feature set as structurally aligned rather than as a reconstruction of the taxonomy.

\begin{table}[!htbp]
	\centering
	\small
	\caption{Supplementary alignment metrics between affinity-propagation clusters and GICS labels. ARI denotes adjusted Rand index, NMI normalized mutual information, AMI adjusted mutual information, and purity the share of firms assigned to the modal GICS class within each cluster.}
	\label{Tab:Affinity_GICS_Metrics}
	\begin{tabular}{lccccc}
		\toprule
		GICS level & Classes & AP clusters & ARI & NMI & Purity \\
		\midrule
		Sector & 11 & 27 & 0.135 & 0.349 & 0.474 \\
		Group & 23 & 27 & 0.182 & 0.438 & 0.388 \\
		Industry & 55 & 27 & 0.140 & 0.531 & 0.294 \\
		Sub-industry & 94 & 27 & 0.121 & 0.578 & 0.260 \\
		\bottomrule
	\end{tabular}
\end{table}
...
\centering
\small
\caption{Research-design timeline. Dates refer to timestamped entries in the version-controlled research log.}
\label{Tab:Research_Timeline}
\begin{tabular}{p{0.24\linewidth}p{0.30\linewidth}p{0.32\linewidth}}
\toprule
Stage & Frozen before evaluation & Role in the paper \\
\midrule
Prediction architecture & v3 residual and ridge-booster objectives locked after the June 20 holdout audit & Confirmatory holdout and transfer evidence \\
Locked hyperparameters & Rank-1 cells from the June 20/29 summaries, reused without re-tuning & Input to all downstream analyses \\
499-firm transfer gate & Declared July 6 before the wide-universe refit & Falsification of a mega-cap-only explanation \\
Veer credit hypothesis & Declared July 6 before the 499-firm veer run & Confirmatory H1/H2 tests \\
High-yield CDS test & Declared July 7 before HY estimation & Confirmatory traded-credit extension \\
Secondary routing tests & Added only after headline outcomes were fixed & Exploratory/subordinate robustness \\
\bottomrule
\end{tabular}
\end{table}
...
Missing entries inside each input window are imputed with a mask-aware Tucker decomposition,
\begin{equation}
...
The Tucker ranks are selected by a held-out-entry validation procedure. For each rolling window, we hide 10\% of observed entries, stratified by feature, fit the mask-aware Tucker model on the remaining observed entries, and score relative Frobenius error only on the hidden entries. The time-mode rank is fixed at $L$, and the firm- and feature-mode ranks are swept over a small grid. This procedure selects ranks $(2,2,2)$ for $L=2$ and $(4,4,4)$ for $L=4$; the $L=2$ choice is the parsimonious one-standard-error selection, while $(4,4,4)$ minimizes the validation error for the one-year lookback. The final imputation step uses SVD initialization, a maximum of 100 iterations, and tolerance $10^{-5}$. Figure~\ref{Fig:Tucker_Imputation} illustrates the decomposition structure. We compute an observed-only RMS scale for each window,
\begin{equation}
...
\begin{equation}
\label{Eq:Ensemble}
\widehat{\mathbf{Y}}_t \;=\; \widehat{\mathbf{B}}_t \;+\; \gamma \cdot s \cdot \langle \mathbfcal{X}_t, \mathbfcal{W} \rangle \in \mathbb{R}^{N \times F},
\end{equation}
where $\widehat{\mathbf{B}}_t$ is the baseline forecast, $\gamma \geq 0$ is a tuned mixing weight, and $s$ inverts the scaling applied to the residual target during training. This architecture answers the question that matters for our purposes: does the multilinear structure add predictive information \emph{beyond} what a strong conventional model already delivers? We instantiate the stack with two baselines of increasing strength.

\textit{Fixed-effects cell.} The first baseline is a within-firm centering analogous to fixed effects in panel econometrics. For each firm--feature pair, we compute a mask-aware temporal mean over the training windows,
\begin{equation}
\mu_{ij} = \frac{\sum_{t} m_{tij}\, y_{tij}}{\sum_{t} m_{tij} + \varepsilon},
\end{equation}
where $m_{tij}$ is the observation indicator and $\varepsilon = 10^{-8}$ prevents division by zero, and set $\widehat{\mathbf{B}}_t = \boldsymbol{\mu}$. The CP target is the centered residual, normalized by its observed root-mean-square
\begin{equation}
s = \sqrt{\frac{1}{|\Omega|}\sum_{(t,i,j)\in\Omega}(y_{tij}-\mu_{ij})^2},
\end{equation}
where $\Omega = \{(t,i,j): m_{tij}=1\}$, yielding $\tilde{y}_{tij} = m_{tij}(y_{tij}-\mu_{ij})/s$; missing entries are set to zero in the scaled target. Optional per-feature target and input rescalings are exposed as tuned binary switches.

\textit{Ridge-booster cell.} The second baseline is the full per-feature ridge regression described in Section~\ref{Section:Baseline_Eval}---the same model that serves as our competitive benchmark. Here the CP tensor is trained on the ridge model's \emph{out-of-fold} residuals: within each training set, an inner three-fold time-series split produces ridge predictions for every window from models that never saw that window, and the CP target is $\tilde{y}_{tij} = m_{tij}(y_{tij} - \hat{y}^{\mathrm{ridge,OOF}}_{tij})/s$. Windows without valid out-of-fold coverage are excluded from CP training rather than back-filled. This out-of-fold discipline prevents the booster from learning the baseline's in-sample optimism, and at prediction time the baseline is refitted on the full training set.
...
% ===== Arrow: W -> output =====
\draw[->, thick] (10.85,1) -- (11.5,1) node[midway, below] {$\times \gamma s \;+\; \widehat{\mathbf{B}}_t$};
...
\end{equation}
where $\widetilde{\mathbf{Y}}_t$ is the zero-filled, centered, RMS-scaled residual target defined above. Because the CP solver does not accept observation masks, missing entries are set to zero prior to fitting; all evaluation at test time remains fully mask-aware.

Hyperparameters are tuned with Optuna \citep{akiba2019optuna} using a tree-structured Parzen estimator over the CP rank $R \in \{1, \ldots, 25\}$, the ridge penalty $\lambda \in [10^{-2}, 10^{5}]$ (log-uniform), the mixing weight $\gamma \in [0, 2]$, and three binary scaling switches (global RMS scaling of the residual target, per-feature target scaling, and per-feature input scaling). The search objective is the incremental fit
\begin{equation}
\Delta R^2 \;=\; R^2\!\left(\mathbf{Y}, \widehat{\mathbf{B}} + \gamma\, \widehat{\mathrm{CP}}\right) - R^2\!\left(\mathbf{Y}, \widehat{\mathbf{B}}\right),
\end{equation}
averaged over a three-fold time-series split of the development block (the first 80\% of rolling windows), where $R^2$ is the pooled mask-aware statistic defined in Section~\ref{Section:Baseline_Eval}. Scoring the \emph{increment} over the cell's own baseline---rather than raw fit---forces the search toward genuinely complementary tensor structure. Each of the four cells (two baselines $\times$ $L \in \{2,4\}$) received several thousand trials distributed across a compute cluster; the top-five trials per cell form a tight plateau, indicating that the search converged. The rank-1 trial per cell is then \emph{locked} and used unchanged in every subsequent analysis in this paper.

\subsection{Ridge benchmark and evaluation protocol}
\label{Section:Baseline_Eval}

The benchmark against which the tensor must prove itself is a per-feature ridge regression on vectorized lag windows; the same model also serves as the baseline inside the ridge-booster cell of Section~\ref{Section:CP_Structured}. Both the CP and ridge models share the same within-firm centering and the same zero-fill convention for missing target cells; the key distinction is that each ridge model is fitted independently for a single feature with its own regularization penalty, whereas the CP tensor shares one low-rank parameterization across all features simultaneously. For each feature $j \in \{1, \ldots, F\}$, let $\mathbf{y}^{(j)} \in \mathbb{R}^{T' N}$ denote the stacked target vector and $\mathbf{X} \in \mathbb{R}^{T' N \times FL}$ the design matrix formed by flattening the lag dimension. The per-feature model is
\begin{equation}
...
\textit{Calendar-fixed holdout.} Final evaluation uses a strict temporal split whose boundary is fixed in calendar time rather than as a fraction of the sample: every window whose prediction target falls in 2021Q1 or later belongs to the test block, and no model, hyperparameter, or scaling constant sees any test-block data. Fixing the boundary by calendar date means that extending the panel forward in time only appends new test quarters; the training set is byte-identical across data refreshes. On the extended panel (fundamentals through 2026Q2), this yields 62 ($L=2$) or 60 ($L=4$) training windows and 21 test windows spanning 2021Q1--2026Q2---a genuine out-of-time extrapolation covering the post-pandemic tightening cycle.
...
Table~\ref{Tab:CP_Ridge_Results} reports pooled, mask-aware test $R^2$ for the four locked cells on the calendar-fixed holdout. In every cell, adding the CP booster improves on its own baseline: by $+0.043$ to $+0.048$ over firm--feature fixed effects, and---the more demanding comparison---by $+0.011$ to $+0.019$ over the fully tuned per-feature ridge. The latter increment is earned against a benchmark that enjoys a separate, independently regularized model for every feature, while the tensor shares a single low-rank parameterization and one global hyperparameter set across all forty. The result is robust to hyperparameter choice: across the top five Optuna trials in each of the four cells, all twenty holdout deltas are positive, and within the $L=2$ cells the top-five spread is below $0.002$ in $R^2$.

\begin{table}[!htbp]
\centering
\caption{Locked-cell holdout results: pooled mask-aware $R^2$ on the calendar-fixed test block (21 windows, 2021Q1--2026Q2; development universe of 50 firms). Each row is the rank-1 Optuna trial of its cell; the CP booster is added to the cell's own baseline.}
\label{Tab:CP_Ridge_Results}
\begin{tabular}{llcccc}
\toprule
Baseline & $L$ & Baseline $R^2$ & Baseline$+$CP $R^2$ & $\Delta$ \\
\midrule
Fixed effects & 2 & 0.721 & \textbf{0.768} & $+0.048$ \\
Fixed effects & 4 & 0.722 & \textbf{0.765} & $+0.043$ \\
...
Where does the gain come from? A per-feature decomposition of the booster delta, repeated for all twenty top-five trials, shows that the improvement concentrates sharply on \emph{non-stationary} features. Using two train-set stationarity metrics per feature---the variance ratio $\mathrm{std}(\Delta x_f)/\mathrm{std}(x_f)$ of the cross-sectional mean series (low values indicate trending behavior) and a normalized trend slope---the correlation between a feature's stationarity and its CP delta is $-0.45$ to $-0.48$ (variance ratio) and $+0.44$ to $+0.50$ (trend slope) in every cell, and the non-stationary tercile of features gains six to twelve times more than the stationary tercile. The features that benefit most are large, trending balance-sheet and income items (total long-term debt, net sales, operating income before depreciation, receivables), while features that are already near-perfectly predicted by their own history (inventories, netted liabilities) gain little. The tensor's value is therefore not diffuse: it captures shared low-rank co-movement precisely where per-feature linear-in-history models are mis-specified.

Two further regularities sharpen the picture. First, the booster delta is largest in test windows with \emph{low} cross-sectional dispersion of targets (correlation $\approx -0.8$ in the ridge cells)---exactly the windows in which firms co-move and a common latent factor exists for the tensor to exploit. Second, the delta does not decay as the forecast origin moves further from the training boundary; if anything it grows slightly across the 21-quarter test block, indicating the structure being captured is persistent rather than an artifact of the split point.
...
A predictive gain established on fifty mega-caps could in principle be an artifact of that particularly well-measured corner of the market. We therefore refit all four locked cells---hyperparameters frozen, no re-tuning of any kind---on the full cross-section selected from the top 499 firms by 2024Q4 market value, using fundamentals through 2026Q2 and the same calendar-fixed split. One selected firm has insufficient cache coverage, so the realized transfer tensor contains 498 firms. The ex ante criterion was directional: if the CP increment collapsed to zero or below in all four cells, that collapse would itself be the finding.

% Numbers: prediction_new/results/v3_holdout_499_20260706/transfer_check_499.csv
\begin{table}[!htbp]
\centering
\caption{Universe transfer at locked hyperparameters: pooled mask-aware test $R^2$ for the 498 firms that survive cache construction from the 499-firm selection universe (22 test windows), against the 50-firm development deltas.}
...
Fixed effects & 2 & 0.691 & 0.709 & $+0.018$ & $+0.048$ \\
Fixed effects & 4 & 0.693 & 0.713 & $+0.020$ & $+0.043$ \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{Tab:Transfer_499} shows the increment survives in all four cells. Against the ridge benchmark, the transfer delta is essentially unchanged from the development universe; against fixed effects it remains clearly positive but shrinks to roughly $40\%$ of its mega-cap size. The multilinear structure is therefore not a mega-cap artifact, though its edge over weak baselines is strongest where fundamentals are best measured---a pattern that recurs throughout the economic analysis below.

Computationally, the tenfold increase in the firm mode makes the stock CP-ALS implementation infeasible: materializing the Khatri--Rao design matrix at 498 firms exhausts memory. We instead exploit the fact that the ALS normal equations factorize exactly---the Gram matrix of the design factorizes into a Hadamard product of small per-mode Gram matrices---so no large intermediate is ever formed. The resulting solver is algebraically identical to the reference implementation (same initialization, same ridge solves, same convergence rule), reduces peak memory by two orders of magnitude, and was verified to reproduce the stock solver's output on the development universe.

\section{Economic Content of Tensor Forecasts}
\label{Section:Economic_Content}

An out-of-sample $R^2$ improvement in accounting space says nothing, by itself, about economic significance. This section asks whether the tensor forecasts carry information that markets price, and if so, where. Our protocol is uniform: for each hypothesis we declare the signal, target, controls, significance criterion, and interpretation \emph{before} running the test, and we report every declared outcome, including the null ones. All tests are run separately in each of the four locked cells; agreement across cells (which share data but differ in architecture) is treated as robustness, not as independent replication.
...
The first battery aligns test-set forecasts with quarterly earnings announcements on the development universe (960 scored announcement events per cell over 21 test quarters). The \emph{ex ante} signal is the magnitude of the tensor's forecast adjustment: the standardized absolute difference between the ensemble and baseline predictions, available before the announcement. Outcomes include announcement-window abnormal returns and post-announcement realized and idiosyncratic volatility over the following 30 trading days, tested per feature--target pair with cross-sectional regressions and a Benjamini--Yekutieli false-discovery-rate control \citep{benjamini2001control} at $q<0.1$ over the full pair grid.

Three findings are unambiguous. First, \emph{abnormal announcement returns are a clean null}: zero of the ex ante return-family pairs survive FDR control, in all four cells. Whatever the tensor knows about fundamentals, it does not predict announcement-window price reactions in mega-caps. Second, the forecast-adjustment magnitude \emph{does} forecast post-announcement volatility beyond lagged realized volatility, but this is architecture-dependent: controlling for pre-event volatility, the signal remains significant ($|t| \geq 2$) in 75 and 69 of 80 pairs in the two ridge-booster cells, against only 13 and 6 in the fixed-effects cells. Third, and decisively for tradeability, the volatility information is \emph{subsumed by option prices}: adding the pre-event at-the-money implied volatility from OptionMetrics as a control eliminates the majority of significant pairs in every cell (at best 29 of 80 survive, in most cells far fewer), and a long--short straddle proxy sorted on the signal against the implied benchmark earns uniformly negative or mixed means---the mechanical variance risk premium, not alpha. In short, the tensor's volatility signal is real but already embedded in option prices for these firms.
...
The event studies use the forecast's \emph{magnitude}; the richer object is the forecast \emph{error}---the direction in which a firm's realized fundamentals veer away from the model's expectation. For each firm, feature, and test quarter we form a studentized forecast surprise: the realized-minus-predicted error, de-biased by an expanding firm-level median of past errors and scaled by an expanding per-feature median absolute deviation (using only past test quarters, with a four-quarter burn-in). Feature-level surprises are aggregated into five themed scores (leverage, earnings, investment, balance-sheet liquidity, and cash flow) following the natural accounting groupings of the 40 features, and a three-quarter rolling mean of each themed score defines a \emph{drift} signal capturing persistent, directional veering. All signals are ex ante: they use only information available after quarter $t$'s announcement to predict outcomes over the following quarter.

On the 499-firm selection universe (roughly 6{,}900 scored events per cell over 18 test quarters), we specified one headline hypothesis before evaluation: \emph{persistent cash-flow over-performance against the tensor forecast predicts improving credit quality}, measured as the change in a naive Merton distance-to-default \citep{bharath2008forecasting} from two trading days before the announcement to 63 trading days after, controlling for the pre-event level and pre-trend. The test is a Fama--MacBeth regression \citep{fama1973risk} across announcement quarters, with a partial rank information coefficient as a secondary check, and a declared success criterion of a positive slope at one-sided $p<0.05$ in at least two of four cells.

% Numbers: veer_report_*_499.txt; PD translation: h1_pd_translation_499.csv
The hypothesis was confirmed; Table~\ref{Tab:H1_DD} reports the four cells. The Fama--MacBeth slope is $+0.020$ in \emph{all four cells}, with one-sided $p = 0.033$ and $0.040$ in the two fixed-effects cells, $p = 0.060$ and $0.071$ in the ridge cells, and a positive partial rank IC ($t \approx +2.1$ to $+2.6$) throughout.

\begin{table}[!htbp]
\centering
\caption{Credit test specified ex ante on the 499-firm selection universe: Fama--MacBeth slope of the post-announcement change in distance-to-default on the cash-flow drift signal, with pre-event level and pre-trend controls ($\approx$6{,}900 events per cell; declared sign positive, one-sided $p$; success bar: $p<0.05$ in $\geq 2$ of 4 cells).}
\label{Tab:H1_DD}
\begin{tabular}{llcccc}
\toprule
...
Fixed effects & 2 & $+0.0202$ & $+2.28$ & 0.040 & $+2.4$ \\
Fixed effects & 4 & $+0.0204$ & $+2.39$ & 0.033 & $+2.6$ \\
\bottomrule
\end{tabular}
\end{table} In economic units, a one-standard-deviation cash-flow drift maps to a distance-to-default improvement of $+0.05$ to $+0.07$; evaluated on the panel's distance-to-default distribution, this is a $29\%$ to $56\%$ \emph{relative} reduction in the model-implied physical default probability, growing with the firm's initial credit quality. For this investment-grade universe, however, absolute default probabilities are tiny, so a large relative change need not be reflected in traded prices; the next subsection tests exactly that. The secondary hypothesis specified ex ante (that the joint set of veer signals adds out-of-sample explanatory power for post-announcement changes in implied volatility) passed formally in three of four cells but with increments of at most $+0.003$ in $R^2$; we treat it as economically negligible, consistent with the option-subsumption result above.

\subsection{Traded credit: an investment-grade null and a high-yield confirmation}
\label{Section:CDS}

% Numbers: v3_holdout_499_20260706/cds_h1_translation_499.csv
To test whether the credit information is priced, we extended the confirmed hypothesis to market credit-default-swap spreads: five-year single-name CDS (senior unsecured, XR/XR14 documentation) from Markit, matched to 169 of the 499 firms with a name-validated link. The declared target was the change in log spread over the same event window, with the same controls; expected sign negative (over-performance tightens spreads). The result is a \emph{clean null in all four cells}: slopes are indistinguishable from zero ($|t| < 0.15$), amounting to $\pm 0.03$ basis points per one-standard-deviation signal at the sample's 56 basis-point median spread. The null is not a matching artifact: re-running the distance-to-default regression on exactly the CDS-covered subsample leaves the slope intact and slightly larger ($+0.024$ to $+0.026$, $t > 2$). The same firm-events whose model-implied credit quality improves show no movement in their traded spread. For investment-grade mega-caps, every traded price we test---equity around announcements, options on volatility, and CDS on credit---already embeds what the tensor forecast errors know; only model-implied quantities respond. Within the investment-grade panel, however, the distance-to-default effect concentrates in the highest spread tercile in all four cells, suggesting the channel matters where default risk is material.

% Universe: build_hy_universe.py; numbers: v3_holdout_hy_20260707/cds_h1_translation_hy.csv
We therefore constructed a dedicated high-yield and crossover universe, again declaring the hypothesis before estimation. From the U.S. five-year senior CDS reference entities in Markit, we selected issuers with median spreads of at least 150 basis points and matched them to listed CRSP/Compustat firms, yielding a universe of 113 names with a median spread of 289 basis points, against 56 in the investment-grade sample.\footnote{Matching is by normalized issuer name, with financing subsidiaries mapped to their listed parents and private or defunct entities excluded. Of 970 reference entities, 298 met the spread threshold and 128 matched, of which 113 had sufficient fundamentals coverage.} Because the universe is defined at 2024Q4 it conditions on survival, which excludes defaulted names and biases against the hypothesis. The full pipeline was re-estimated at locked hyperparameters; the CP increment remains positive in three of four cells, turning negative in the fixed-effects $L=4$ cell, consistent with low-rank structure weakening as fundamentals become noisier down the credit spectrum.

\begin{table}[!htbp]
\centering
\caption{High-yield CDS test specified ex ante: Fama--MacBeth slope of the post-announcement change in log five-year CDS spread on the cash-flow drift signal, with pre-event spread level and pre-trend controls (900 events, 90 CDS-covered firms, 13 quarters per cell; declared sign negative, one-sided $p$). Basis-point impact evaluated at the 282 bp sample median spread.}
\label{Tab:HY_CDS}
\begin{tabular}{llcccc}
\toprule
Baseline & $L$ & Slope & $t$ & One-sided $p$ & bp per $1\sigma$ \\
\midrule
Ridge         & 2 & $-0.0113$ & $-2.21$ & 0.024 & $-6.3$ \\
Ridge         & 4 & $-0.0151$ & $-2.74$ & 0.009 & $-6.8$ \\
Fixed effects & 2 & $-0.0112$ & $-2.34$ & 0.019 & $-6.9$ \\
Fixed effects & 4 & $-0.0104$ & $-1.96$ & 0.037 & $-6.4$ \\
\bottomrule
\end{tabular}
\end{table}

We subject this CDS result to a small robustness battery, summarized in Table~\ref{Tab:HY_CDS_Robust}. The sign is stable: the cash-flow drift coefficient is negative in every cell under log-spread changes at 21-, 42-, and 63-trading-day horizons, under raw-basis-point spread changes, after one-quarter-at-a-time deletions, and within each pre-event spread tercile. Statistical strength is not uniform. The 42-day window is strongest; raw spread changes preserve the economic sign and imply larger basis-point moves but are noisier; 5\% winsorization and dropping 2022 tightening quarters attenuate significance in some cells. The tercile split is economically useful: the basis-point effect is largest in the widest-spread tercile, consistent with the signal mattering most where default risk is closest to the money.

\begin{table}[!htbp]
\centering
\caption{High-yield CDS robustness summary. Entries aggregate the four locked cells. The sign column reports the number of cells with the expected negative slope; the basis-point range is the implied effect of a one-standard-deviation cash-flow drift at the relevant median spread.}
...
Log spread, +63td & 4/4 & $-6.3$ to $-6.9$ & headline window \\
...
Table~\ref{Tab:HY_CDS} reports the outcome specified ex ante: the hypothesis is confirmed in \emph{all four cells} against a declared bar of two. In crossover and high-yield names, a one-standard-deviation persistent cash-flow over-performance against the tensor forecast predicts approximately $2.3\%$ of spread tightening---about 6.5 basis points at the median spread---over the following quarter, controlling for the spread level and its pre-trend, with negative partial rank ICs in all four cells. The same signal that moved only a model-implied credit metric in the investment-grade universe moves the traded market price of credit here. Together, the two CDS experiments are consistent with faster incorporation of this information in mega-cap investment-grade markets and slower incorporation where default risk is material. Given the small sample (90 firms, 13 quarters) and the correlation across cells, we count this as a single confirmation rather than four independent ones.

Three secondary results complete the picture. First, an options-tradeability battery was specified ex ante on the high-yield universe, using the same straddle-proxy long--short construction that failed at mega-caps: the primary criterion---a positive and bootstrap-significant long--short straddle mean in at least two of four cells---was met exactly at the declared bar, but only in the two $L=4$ cells, and the secondary criterion (per-pair significance incremental to implied volatility) failed in all four. We interpret this as a weak, configuration-dependent effect and do not claim a second tradeable result. Second, the drift signal does \emph{not} predict forward equity returns in the 499-firm selection universe (Fama--MacBeth $t < 0.8$ everywhere; tercile long--short spreads positive but insignificant), closing the equity route in this universe for both the magnitude and error-direction signals. Third, forecast errors are essentially idiosyncratic: clustering firms by error co-movement recovers no sectoral structure (adjusted Rand index versus GICS below $0.04$ in every cell) and the first principal component explains under $4\%$ of error variance---the veer signals are firm-specific news, not a missed common factor.

\section{Conclusion}

This paper develops a tensor-based framework for firm fundamentals in three movements. First, treating the panel of accounting data as a third-order tensor, we constructed the Market Fundamentals Index from the temporal factors of a mask-aware Tucker decomposition and showed that this fundamentals-based volatility measure is statistically dependent on price-based chaos (FCIX), with independence rejected at the $1\%$ level.

Second, we showed the same tensor structure is predictive. A low-rank CP tensor regression, deployed as a booster over per-feature ridge and fixed-effects baselines with strict out-of-fold discipline, improves out-of-time forecasts of next-quarter fundamentals in every locked configuration, on both a 50-firm development universe and the wider 499-firm selection universe at frozen hyperparameters. The gain concentrates precisely on non-stationary, trending line items and on periods of cross-sectional co-movement---the regimes where per-feature linear models are mis-specified and shared low-rank structure exists to be exploited.

Third, we asked whether markets price what the tensor knows, using hypotheses, criteria, and interpretations specified before evaluation and timestamped in the research log. In mega-cap investment-grade names, every traded price we examined---announcement returns, option-implied volatility, and CDS spreads---already embeds the information in the tensor's forecasts and forecast errors; only model-implied quantities (a Merton distance-to-default) respond, and a large relative improvement in an infinitesimal default probability is worth little at the spread. In crossover and high-yield credit, where default risk is material, the ex ante CDS test confirmed in all four model configurations that persistent cash-flow over-performance against the tensor forecast predicts economically meaningful spread tightening---roughly 6.5 basis points per standard deviation per quarter at the median spread---controlling for spread level and momentum. The pattern is consistent with slower incorporation of this information lower down the credit spectrum.

The analysis has candid limitations that chart the path forward: the high-yield sample is short and survivorship-gated, the four model cells are correlated rather than independent replications, and the equity and options routes remain closed or fragile. Natural extensions include longer high-yield histories, dead-firm inclusion to remove the survivorship gate, and routing the veer signals to credit instruments directly tradeable at scale.
